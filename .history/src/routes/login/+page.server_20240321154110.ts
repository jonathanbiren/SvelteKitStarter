...
import bcrypt from 'bcrypt'; async function authenticateUser(email: string, password: string) : boolean{ const prisma = new PrismaClient(); const user: UserSmall | null = prisma.userSmall.findUnique({
			where: {email},
		}) 
