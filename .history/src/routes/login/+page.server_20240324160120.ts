...
import { authenticateUser } from '../../../.history/src/lib/db_20240314115045';
...
async function authenticateUserLDAP(email: string, password: string): Promise<boolean> {
	//This is where you would implement the LDAP authentication
...
export const actions: Actions = {
	login: async ({ request, cookies }) => {
		...
		const authenticated = await authenticateUserLocal(email, password);
