...
async function authenticateUser(email: string, password: string) : UserSmall | null {
		const prisma = new PrismaClient();
		const user: UserSmall = prisma.userSmall.findUnique({
			where: {email},
		}) 

		if(user) {
		const isMatch: boolean = await bcrypt.compare(password, user.pass)
		} else {
